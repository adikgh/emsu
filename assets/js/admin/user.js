...
$(document).ready(function() {
   ...
   // cours add block
	...
	// cours add block
	$('.reser_add_pop').click(function(){
		$('.reser_add_block').addClass('pop_bl_act');
		$('#html').addClass('ovr_h');
	})
	$('.reser_add_back').click(function(){
		$('.reser_add_block').removeClass('pop_bl_act');
		$('#html').removeClass('ovr_h');
	})
	// 
	$('.btn_reser_add').click(function () { 
		if (!$('.ins_reser_name').attr('data-sel') && !$('.ins_reser_phone').attr('data-sel')) mess('Форманы толтырыңыз')
		else {
			$.ajax({
				url: "/admin/reser/get.php?reser_add",
				type: "POST",
				dataType: "html",
				data: ({
